                ...
                for (int v=1; v<1000; v++) {
                    ...
                    for (double t=0.1; t<tiempo; t+=0.01) {
                        ...
                        if ((Yd<0)||(Yo<0)) continue;
                            ...
                            // En este punto ya se sabe que puede evitar el impacto
                            // Ahora se sigue iterando en el tiempo para saber si podria dañar al cañon ofensivo.
                            for (double t2=t; t2<(2*tiempo); t2+=0.01) {
                                // Se evalua la distancia al cañon ubicado en la posicion 0, Ho
                                if (distancia(0,Ho,posXd(v,45*pi/180,t+2.5),posYd(v,45*pi/180,t+2.5)) <=rangoD) {
                                    danio = true;
                                    break;
                                }
                            }
                            // Si hace daño se debe seguir evaluando
                            if (danio) continue;
                            tiempo = t;
                            Vod = v;
                            flag = true;
